...
class MarkdownProcessor(object):
    ...
    def on_source_read(self, app, docname, source):
        ...
        try:
            ...
            newlines = []
            for line in source[0].split(u"\n"):
                if self._is_heading_line(line):
                    prev = newlines[-1]
                    inedexline = u".. index:: {0}\n".format(prev)
                    newlines.insert(-1, inedexline)
                newlines.append(line)

            # insert contents directive
            newlines.insert(3, "")
            newlines.insert(4, ".. contents::")
            newlines.insert(5, "   :local:")
            newlines.insert(6, "")
            source[0] = "\n".join(newlines)
